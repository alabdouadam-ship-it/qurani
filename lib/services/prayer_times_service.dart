...
class PrayerTimesService {
  ...
  static Future<Map<String, DateTime>?> getTimesForDate({
    ...
    DateTime? parse(String key) {
      ...
      final hhmm = raw.split(' ').first; // drop timezone part if any
  ...
  // Removed legacy fixed-method resolver. Use resolveMethodForRegionFromPosition/resolveMethodFromLastKnownPosition instead.
  ...
  // Prayer method mapping by country (case-insensitive)
  ...
  // Resolve Aladhan method by current region (from a given position). Default 3 if unknown.
  ...
  // Resolve method using last known position; default 3.
  ...
  static Future<void> maybeRefreshCacheOnLaunch() async {
    // Refresh every 10 days using last known position without prompting
    ...
    if (lastAttemptDay == todayKey) return; // already tried today
    ...
    try {
      ...
      // Schedule remaining adhans for today based on stored toggles and selected sound
      try {
